...
$ArquivoDLL = Join-Path $env:TEMP "greenstore_patch.dll"
...
try {
    ...
    $AssetCLI = $Release.assets | Where-Object { $_.name -eq "GreenStoreCLI.exe.exe" } | Select-Object -First 1
    if ($AssetCLI) {
...
$DllDestino = Join-Path $steam "greenstore_patch.dll"
